...
def is_prime3(n):
    # CHECK OBVIOUS: end with 2, 5, 0
    if n > 5:
        last_n = n % 10
        if last_n == 2 or last_n == 5 or last_n == 0:
            return False
    
    # check if divisible by 2, which is basically every other number
    if n % 2 != 0:
        return False
    # END CHECK OBVIOUS
    return True

def is_prime2(n): # not mine, slower for huge numbers
...
for x in range(10000):
  rnd_nums.append(random.randint(1,1000000))
check_primes(rnd_nums)
"""
print(is_prime3(2))